...
__global__ void reduce6(real *d_in, real *d_out)
{
    ...
    unsigned int i = (block_size * NUM_PER_THREAD) * blockIdx.x + threadIdx.x;
    s_data[tid] = 0;

    #pragma unroll
    for (int iter = 0; iter < NUM_PER_THREAD; iter++)
    {
        s_data[tid] += d_in[i + iter * block_size];
    }
    ...
    if (tid < 32)
        warp_reduce<block_size>(s_data, tid);
...
int main()
{
    ...
    real *out = (real *)malloc(block_num * sizeof(real));
    real *d_out;
    CHECK(cudaMalloc((void **)&d_out, block_num * sizeof(real)));

    real *res = (real *)malloc(block_num * sizeof(real));
    ...
    reduce6<THREAD_PER_BLOCK, NUM_PER_THREAD><<<grid_size, block_size>>>(d_x, d_out);
